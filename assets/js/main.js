...
(function($) {
	...
	$.ajax({url: "https://corona-virus-stats.herokuapp.com/api/v1/cases/general-stats", success: function(result){
		...
		$.ajax({url: "https://api.covid19api.com/summary", success: function(result){
	...
	$.ajax({url: "https://spreadsheets.google.com/feeds/cells/1yvmeAtpxonqmJ7OlO2gRBt2lUVnk3baeIHFu0M9ZLxo/1/public/values?alt=json", success: function(result){
		var table = "";
	...
	$.ajax({url: "https://spreadsheets.google.com/feeds/cells/1Ebxlw5HEqzr1JEO4atHZiYGweUUQ6gIPBECDjuvkHR8/1/public/values?alt=json", success: function(result){
		var table = "";
		for (var i = 0; i < result["feed"]["entry"].length-1; i=i+5){
		...
		document.getElementById("insertPhase1Here").innerHTML = table;
		  ...
		  $.ajax({url: "https://spreadsheets.google.com/feeds/cells/1PINwxQbZr14xz_TfODu0_XQNRPuAlsGgIqcVbFjGOn8/1/public/values?alt=json", success: function(result){
			var table = "";
			for (var i = 0; i < result["feed"]["entry"].length-1; i=i+5){
				table += "<tr>";
				table +=  '<td style="font-size: 0.8em;">'+result["feed"]["entry"][i]["content"]["$t"]+'</td>';
